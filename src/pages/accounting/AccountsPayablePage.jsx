...
export default function AccountsPayablePage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <Toaster />
      <div>
        <h1 className="text-3xl font-bold mb-4">Accounts Payable</h1>
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white p-4 rounded shadow">
            <p className="text-gray-600 text-sm">Total Bills</p>
            <p className="text-2xl font-bold">{formatCurrency(bills.reduce((sum, b) => sum + b.amount, 0))}</p>
          </div>
          <div className="bg-white p-4 rounded shadow">
      ...
      <div className="bg-white p-6 rounded shadow">
        <h3 className="text-lg font-semibold mb-4">Outstanding Bills</h3>
        <DataTable 
          columns={columns} 
          data={bills.filter(b => b.status !== 'paid')} 
          rowsPerPage={10}
          actions={[
            { label: 'Pay', onClick: handlePayBill, color: 'green' }
          ]}
        />
      </div>
    </div>
